...
describe('chatRoomRouter', () => {
  ...
  describe('get', () => {
    ...
    it('should return all chatrooms if the user is admin', async () => {
      jest
        .spyOn(ChatRoom, 'find')
        .mockImplementation(() => Promise.resolve([chatRoom, chatRoom]));
      jest
        .spyOn(functions, 'resolveAndMapUserNamesFromQuery')
        .mockImplementation(() =>
          Promise.resolve([
            { ...chatRoom, userName: 'testUserName1' },
            { ...chatRoom, userName: 'testUserName2' },
          ])
        );
